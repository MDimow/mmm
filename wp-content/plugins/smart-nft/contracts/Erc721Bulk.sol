...
contract Smartnft_Pro_ERC721_Bulk is  ERC721, ERC721Enumerable, ERC721URIStorage {
  ...
  function mintBatch( uint256 totalAmount, string[] memory _uris ) public payable {
	 uint256 newTokenId;
	 for( uint256 i = 1; i <= totalAmount; i++ ) {
          ...
          //`_safeMint, _setTokenURI` fn is comming from ERC721 contract that we extend. 
	      _safeMint(msg.sender, newTokenId);
	      _setTokenURI( newTokenId, _uris[ i - 1 ] );
	}	
  }
