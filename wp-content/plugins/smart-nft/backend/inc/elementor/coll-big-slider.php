...
class SMARTNFT_COLLECTION_BIG_SLIDER extends \Elementor\Widget_Base 
{
	public function get_name() {
		return 'smartnft_collection_big_slider';
	}

	public function get_title() {
		return esc_html__( 'Collection Full slider', WP_SMART_NFT );
	}
	...
	$this->start_controls_section(
			'btn_style_section',
			[
				'label' => esc_html__( 'Button style', WP_SMART_NFT ),
				'tab' => \Elementor\Controls_Manager::TAB_STYLE,
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .collections-grid .collection-grid-classic > button.right-btn',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => \Elementor\Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .collections-grid .collection-grid-classic > button.right-btn' => 'color: {{VALUE}}',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'type' => \Elementor\Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .collections-grid .collection-grid-classic > button.right-btn' => 'background: {{VALUE}}',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px'],
				'selectors' => [
					'{{WRAPPER}} .collections-grid .collection-grid-classic > button.right-btn' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px'],
				'selectors' => [
					'{{WRAPPER}} .collections-grid .collection-grid-classic > button.right-btn' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px'],
				'selectors' => [
					'{{WRAPPER}} .collections-grid .collection-grid-classic > button.right-btn' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
    ...
    <?php foreach( $collections as $collection ): ?>
            ...
            <a href="<?php echo $collection['permalink']; ?>">
                <div class="collection-grid-classic">
                        <?php if( !empty ( $collection['bannerImg'] ) ): ?>
                            <img class="profile" src="<?php echo   $collection['bannerImg'];?>" alt="<?php echo $collection['name']; ?>">
                        <?php else: ?>
                        ...
                        <div class="info">
							<h2 class="name">
								...
								<?php if( !empty($collection["verified"] ) && $collection["verified"] == true ): ?>
									<img src="<?php echo FRONTEND_MEDIA_URL . "verified.svg" ?>" alt="verified">
								<?php endif; ?>
							</h2>
                            <div>
                                <p><?php echo $translation['items']; ?> </p>
                                <p><?php echo $collection['count']; ?></p>
                                <p><?php echo $translation['floor']; ?></p>
                                <span><?php echo $collection['flour_price']; ?> <?php echo $collection['network']['currencySymbol']; ?></span>
                            </div>
                        </div>
						<button class="right-btn"><?php echo $translation['view_collection']; ?></button>
                </div>
            </a>
      ...
      <script>
		...
		(()=>{
			...
			if(typeof elementor !== "undefined"){
				elementor.hooks.addAction( 'panel/open_editor/widget/smartnft_collection_big_slider', function() {
					initSlider();
				});
			}
