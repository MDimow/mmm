    ...
    if ( ! class_exists( 'Redux_Tracking' ) ) {
        ...
        class Redux_Tracking {
            ...
            public function load( $parent ) {
                ...
                $this->options             = get_option( 'legacy_framework-tracking' );
                ...
                if ( ! isset( $this->options['hash'] ) || ! $this->options['hash'] || empty( $this->options['hash'] ) ) {
                    ...
                    update_option( 'legacy_framework-tracking', $this->options );
                }
            ...
            function tracking_request() {
                ...
                $content = '<h3>' . __( 'Help improve Our Panel', 'legacy_framework' ) . '</h3>';
                $content .= '<p>' . __( 'Please helps us improve our panel by allowing us to gather anonymous usage stats so we know which configurations, plugins and themes to test to ensure compatibility.', 'legacy_framework' ) . '</p>';
                $opt_arr = array(
                    'content'  => $content,
                    'position' => array( 'edge' => 'top', 'align' => 'center' )
                );
                $button2 = __( 'Allow tracking', 'legacy_framework' );

                $function2 = 'redux_store_answer("yes","' . $nonce . '")';
                $function1 = 'redux_store_answer("no","' . $nonce . '")';

                $this->print_scripts( $id, $opt_arr, __( 'Do not allow tracking', 'legacy_framework' ), $button2, $function2, $function1 );
            }
            ...
            function newsletter_request() {
                ...
                $content = '<h3>' . __( 'Welcome to the Redux Demo Panel', 'legacy_framework' ) . '</h3>';
                $content .= '<p><strong>' . __( 'Getting Started', 'legacy_framework' ) . '</strong><br>' . sprintf( __( 'This panel demonstrates the many features of Redux.  Before digging in, we suggest you get up to speed by reviewing %1$s.', 'legacy_framework' ), '<a href="' . 'http://' . 'docs.reduxframework.com/legacy_framework/getting-started/" target="_blank">' . __( 'our documentation', 'legacy_framework' ) . '</a>' );
                $content .= '<p><strong>' . __( 'Redux Generator', 'legacy_framework' ) . '</strong><br>' . sprintf( __( 'Want to get a head start? Use the %1$s. It will create a customized boilerplate theme or a standalone admin folder complete with all things Redux (with the help of Underscores and TGM). Save yourself a headache and try it today.', 'legacy_framework' ), '<a href="' . 'http://' . 'generate.reduxframework.com/" target="_blank">' . __( 'Redux Generator', 'legacy_framework' ) . '</a>' );
                $content .= '<p><strong>' . __( 'Redux Extensions', 'legacy_framework' ) . '</strong><br>' . sprintf( __( 'Did you know we have extensions, which greatly enhance the features of Redux?  Visit our %1$s to learn more!', 'legacy_framework' ), '<a href="' . 'http://' . 'reduxframework.com/extensions/" target="_blank">' . __( 'extensions directory', 'legacy_framework' ) . '</a>' );
                $content .= '<p><strong>' . __( 'Like Redux?', 'legacy_framework' ) . '</strong><br>' . sprintf( __( 'If so, please %1$s and consider making a %2$s to keep development of Redux moving forward.', 'legacy_framework' ), '<a target="_blank" href="' . 'http://' . 'wordpress.org/support/view/plugin-reviews/legacy_framework">' . __( 'leave us a favorable review on WordPress.org', 'legacy_framework' ) . '</a>', '<a href="' . 'https://' . 'www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=N5AD7TSH8YA5U" target="_blank">' . __( 'donation', 'legacy_framework' ) . '</a>' );
                $content .= '<p><strong>' . __( 'Newsletter', 'legacy_framework' ) . '</strong><br>' . __( 'If you\'d like to keep up to with all things Redux, please subscribe to our newsletter', 'legacy_framework' ) . ':</p>';
                $content .= '<form action="' . 'http://' . 'reduxframework.us7.list-manage2.com/subscribe/post?u=564f5178f6cc288064f332efd&amp;id=ace5bbc1f9&SOURCE=panel" method="post" id="mc-embedded-subscribe-form" name="mc-embedded-subscribe-form" class="validate" target="_blank" novalidate><p style="text-align: center;"><label for="mce-EMAIL">' . __( 'Email address', 'legacy_framework' ) . ' </label><input type="email" value="" name="EMAIL" class="required email" id="mce-EMAIL"><input type="hidden" value="panel" name="SOURCE">&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" value="' . __( 'Subscribe', 'legacy_framework' ) . '" name="subscribe" id="mc-embedded-subscribe" class="button button-primary"></p></form>';
                $opt_arr = array(
                    'content'      => $content,
                    'position'     => array( 'edge' => 'top', 'align' => 'center' ),
                    'pointerWidth' => 450
                ...
                $this->print_scripts( $id, $opt_arr, __( 'Close', 'legacy_framework' ), false, '', $function1 );
            }
        ...
        function redux_allow_tracking_callback() {
            ...
            if ( wp_verify_nonce( $_REQUEST['nonce'], 'redux_activate_tracking' ) ) {
                $options = get_option( 'legacy_framework-tracking' );
                ...
                if ( update_option( 'legacy_framework-tracking', $options ) ) {
                    die( '1' );
                } else {
                    die( '0' );
                }
